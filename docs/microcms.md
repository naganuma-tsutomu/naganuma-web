...
| `content` | 本文（リッチエディタ） | リッチエディタ | いいえ |
| `contentMarkdown` | 本文（Markdown） | テキストエリア | いいえ |

`content`と`contentMarkdown`のどちらか一方には本文を入力してください。両方に値がある場合は`contentMarkdown`を優先します。既存のリッチエディタ記事は変更せず、そのまま表示できます。

既存APIへMarkdown対応を追加する場合は、`content`の「必須」をオフにしてから、フィールドID `contentMarkdown` のテキストエリアを追加します。既存フィールドの種類を変更したり削除したりする必要はありません。

### Markdown本文の入力方法

- front matterの`title`、`description`、`thumbnail`は本文表示から除外されますが、各microCMSフィールドへ自動転記はされません。それぞれ管理画面の専用フィールドにも入力してください。
- GitHub Flavored Markdownの見出し、リスト、引用、リンク、画像、コードブロック、表、打ち消し線に対応します。
- 表やリストの各行間に空行が入っていても、表示時に連続する要素として整形します。
- Markdown内のHTMLも最終的にサニタイズし、スクリプト、イベント属性、iframe、危険なURL、任意の装飾スタイルを除去します。
- 長い表はPCでは本文幅に合わせ、スマートフォンでは横スクロールで表示します。

`description` はカードで最大3行表示します。`thumbnail` がない記事は既定の画像を表示します。
...
2. タイトル・説明文と、`contentMarkdown`または`content`の本文を入力し、任意でサムネイルを設定します。
...
APIレスポンスはNext.jsで60秒キャッシュします。期限後のアクセスで更新する方式なので、公開・編集直後は以前の内容が表示される場合があります。初回アクセスが更新を開始し、更新後のアクセスから新しい内容になります。Webhookによる即時更新は未実装です。

## 4. 下書きの画面プレビューを設定する

microCMSの `projects` APIで「API設定」→「画面プレビュー」を開き、遷移先URLに次を設定します。

```text
https://naganuma-web.com/api/preview?contentId={CONTENT_ID}&draftKey={DRAFT_KEY}
```

コンテンツ編集画面の「画面プレビュー」を押すと、下書き内容を反映した `/projects/{コンテンツID}` が開きます。プレビュー中は記事上部に `PREVIEW` と表示します。

- `draftKey` は専用エンドポイントで15分間有効なHttpOnly Cookieへ移し、記事URLやGoogle Analyticsには渡しません。
- プレビューのmicroCMSリクエストはキャッシュしないため、編集後に再読み込みすると最新の下書きを確認できます。
- Cookieは `/projects` 配下だけに送信されます。別のコンテンツIDには適用されません。
- 通常の記事取得と同じGET権限のAPIキーを使用できます。「下書き全取得」権限は不要です。
...
テストはNode.js 22.6以降のTypeScript型除去機能を利用します。API認証、設定不足、APIエラー、Markdown変換、本文HTMLのサニタイズをモックで確認します。実サービスとの接続は環境変数を設定してから確認してください。
